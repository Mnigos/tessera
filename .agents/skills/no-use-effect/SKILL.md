...
The only place `useEffect` may appear directly is inside reusable custom hooks like `useMountEffect`. Components must never import or call `useEffect`.
...
For expensive calculations, keep the work pure and compute it during render. In React Compiler codebases, do not add manual memoization by default:
...
### Rule 2: Use React Query for data fetching

Effect-based fetching creates race conditions and reinvents caching. Fetch with React Query instead.
...
**Smell test:** You're reaching for `fetch()` in a component, or reimplementing caching/retries/cancellation instead of `useQuery` / `useInfiniteQuery` / `useMutation`.
...
function handleMeasuredNode(node: HTMLDivElement | null) {
  if (node !== null) {
...
1. **Can I compute it during render?** -> Derive it inline
2. **Is it triggered by a user action?** -> Event handler
3. **Am I fetching data?** -> React Query
