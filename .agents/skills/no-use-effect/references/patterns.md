...
- [Expensive Calculations During Render](#expensive-calculations-during-render)
...
**When is a calculation expensive?** Measure with `console.time`. In React Compiler codebases, keep the computation pure and let the compiler optimize it before adding manual memoization.
...
React Query handles caching, deduplication, cancellation, retries, background refetching, and stale state for you.
...
// GOOD: React Query handles cancellation
function Profile({ userId }) {
...
function handleChartNode(node: HTMLDivElement | null) {
  if (node === null) return;
  const chart = new Chart(node, config);
  return () => chart.destroy(); // cleanup when node detaches
}
return <div ref={handleChartNode} />;
```

**Note:** React 19 supports cleanup return values from callback refs. For earlier versions, store the cleanup in a ref inside a custom hook. If the ref callback becomes expensive, isolate the DOM-owning component instead of adding manual memoization.
