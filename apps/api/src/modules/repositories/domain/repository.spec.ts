...
describe('repository domain mapper', () => {
	...
	test('maps safe external source metadata to API output', () => {
		...
		expect(toRepositoryOutput(repository).repository.externalSource).toEqual({
			...
			cutoverActorUserId: undefined,
			cutoverAt: undefined,
			cutoverFromMirrorMode: undefined,
			githubPushBackEnabled: false,
			githubPushBackStatus: 'idle',
			githubPushBackStartedAt: undefined,
			githubPushBackSucceededAt: undefined,
			githubPushBackFailedAt: undefined,
			githubPushBackFailureReason: undefined,
			createdAt: new Date('2026-05-12T00:00:00Z'),
			updatedAt: new Date('2026-05-12T00:01:00Z'),
		})
	})
