...
pub fn router(config: Config) -> Router {
    let api_authorization_url_configured = !config.api_authorization_url.trim().is_empty();
    let api_token_present = config
        .api_authorization_token
        .as_ref()
        .is_some_and(|token| !token.trim().is_empty());

    if api_authorization_url_configured != api_token_present {
        panic!(
            "invalid smart-http auth config: GIT_API_AUTHORIZATION_URL and GIT_API_AUTHORIZATION_TOKEN must be set together"
        );
    }
    ...
    tracing::info!(
        ...
        api_authorization_url_configured,
        api_token_present,
